...
void PaintWidget::addSimpleFrame(int position)
{
    painter.layers[painter.currentLayer]->addFrameForLayer( position, true );
    painter.setFrame( position, false );
    emit frameChanged(qreal(position));
    emit StateChanged("Add frame");
}

void PaintWidget::copyNextFrame(int position, int copy_position)
{
    painter.layers[painter.currentLayer]->addFrameForLayer2( position, true, copy_position );
    painter.setFrame( position, false );
    emit frameChanged(qreal(position));
    emit StateChanged("Add frame");
}
